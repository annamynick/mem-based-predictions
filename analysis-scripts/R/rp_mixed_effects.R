...
model1 <- lmer(rt ~ condition + (1 | subject_id) + (1 | targetName), data = tda, REML = F)
summary(model1)
anova(model1)
...
tda = read.csv("~/Dropbox_Lab/projects/pattern-completion/analysis-scripts-CB-s2/memory-based-predictions/analysis-scripts/tda_lf_Experiment2.csv")
...
tda3 = read.csv("~/Dropbox_Lab/projects/pattern-completion/analysis-scripts-CB-s2/memory-based-predictions/analysis-scripts/tda_lf_Experiment3.csv")
...
# Using random intercepts for subjects and scenes; 
model3 <- lmer(rt ~ primeCondition * arrowValidity + (1 | subject_id) + (1 | targetName), data = tda3, REML = F)
summary(model3)
anova(model3)
...
tda = read.csv("~/Dropbox_Lab/projects/pattern-completion/analysis-scripts-CB-s2/memory-based-predictions/analysis-scripts/tda_lf_Experiment6.csv") #Experiment 4 in manuscript is named  Experiment 6 in the code 
